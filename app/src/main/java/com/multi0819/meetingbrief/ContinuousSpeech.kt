...
class ContinuousSpeech(context:Context,private val onPartial:(String)->Unit,private val onFinal:(String)->Unit,private val onState:(Boolean)->Unit,private val onError:(String)->Unit):RecognitionListener {
 ...
 private val accumulator=TranscriptAccumulator();private var active=false
 ...
 fun start(){if(active)return;active=true;onState(true);listen()}
 fun stop(){active=false;onState(false);onPartial("");recognizer.stopListening()}
 ...
 override fun onPartialResults(v:Bundle?){onPartial(v?.getStringArrayList(SpeechRecognizer.RESULTS_RECOGNITION)?.firstOrNull().orEmpty())}
 override fun onResults(v:Bundle?){val raw=v?.getStringArrayList(SpeechRecognizer.RESULTS_RECOGNITION)?.firstOrNull().orEmpty();accumulator.append(raw)?.let(onFinal);onPartial("");restart()}
